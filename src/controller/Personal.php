...
class Personal
{
    ...
    {
        $admins = new Admins();
        $admins = $admins->hidden()->get(AdminsAuth::id());
        if ($request->isAjax()) {
            $data = $request->param();
            ...
            try {
                ...
                isset($data['avatar']) && $data['avatar'] ? $admins->avatar = $data['avatar'] : '';
                if ($data['password']) {
                    $salt = random_str(20);
        ...
        $form->upload()->label('头像')->name('avatar')->field('image')->url(url('thans\layuiAdmin\controller\Upload@image'))->value($admins->avatar);
